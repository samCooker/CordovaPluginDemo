    ...
    public static void doEnqueue(final Request request, final HttpCallback callback) {
        getOkHttpClient().newCall(request).enqueue(new Callback() {
            ...
            public void onResponse(Call call, Response response) throws IOException {
                try {
                    if (response.isSuccessful()) {
                        OkHttpUtils.sendSuccessResultCallback(response.body().string(), callback);
                    } else {
                        OkHttpUtils.sendFailResultCallback(response.code(), response.message(), callback);
                    }
                } catch (Exception e) {
        ...
        getOkHttpClient().newCall(request).enqueue(new Callback() {
            ...
            public void onResponse(Call call, Response response) {
                ...
                try {
                    ...
                    OkHttpUtils.sendSuccessResultCallback(new ResultDesc(0, "下载成功", file.getAbsolutePath()), callback);
                } catch (IOException e) {
                    e.printStackTrace();
                    OkHttpUtils.sendFailResultCallback(-1, e.getMessage(), callback);
                } finally {
